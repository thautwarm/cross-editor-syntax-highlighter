...
def codegen(analyzer: Analyzer, cg_options: CodeGenOptions, lang_name: str, stmts: List[definition]) -> List[Tuple[str, Doc]]:
    ...
    abandoned_names : Any = of_array(["and", "as", "assert", "asr", "begin", "class", "constraint", "do", "done", "downto", "else", "end", "exception", "external", "false", "for", "fun", "function", "functor", "if", "in", "include", "inherit", "initializer", "land", "lazy", "let", "lor", "lsl", "lsr", "lxor", "match", "method", "mod", "module", "mutable", "new", "nonrec", "object", "of", "open", "or", "private", "rec", "sig", "struct", "then", "to", "true", "try", "type", "val", "virtual", "when", "while", "with"], ObjectExpr123())
    class ObjectExpr124:
        @property
        def Compare(self) -> Any:
    ...
    ocaml_var_ident_descr : NameMangling_IdentifierDescriptor = NameMangling_IdentifierDescriptor__WithNameEnv_Z7613F24B(NameMangling_IdentifierDescriptor_Create_Z48C5CCEF(arrow_126, lambda i_1, c_1, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: to_text(interpolate("_X%P()_", [i_1])) if (is_digit(c_1)) else (c_1.lower() if (is_upper(c_1)) else (to_text(interpolate("_%P()_", [ord(c_1)]))))), NameMangling_nameEnv(of_list(map_1(lambda tuple, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: tuple[1], global_scope), ObjectExpr125())))
    class ObjectExpr127:
        @property
        def Compare(self) -> Any:
    ...
    def cg_expr(scope: FSharpList[Tuple[str, str]], curr_expr: expr, analyzer: Analyzer=analyzer, cg_options: CodeGenOptions=cg_options, lang_name: str=lang_name, stmts: List[definition]=stmts) -> Doc:
        ...
        if match_value_1.tag == 6:
            ...
            def arrow_139(scope: FSharpList[Tuple[str, str]]=scope, curr_expr: expr=curr_expr) -> Iterable[Tuple[str, str]]:
                def arrow_138(match_value_3: Tuple[str, monot]) -> Iterable[Tuple[str, str]]:
                    arg : str = match_value_3[0]
                    return singleton((arg, mangle(ocaml_var_ident_descr, arg)))
                
                return collect(arrow_138, match_value_1.fields[0])
            
            code : Doc = cg_expr(append(to_list(delay(arrow_139)), scope), match_value_1.fields[1])
        ...
        if stmt.tag == 2:
            ...
            def arrow_147(stmt: definition=stmt) -> Iterable[Doc]:
                def arrow_146(match_value_4: Tuple[position, production_1]) -> Iterable[Doc]:
                    ...
                    return singleton(arrow_145())
                
                return collect(arrow_146, decl["define"])
            
            body_4 : Doc = align(vsep(map_indexed(lambda i_7, e_1, stmt=stmt: Doc_op_Addition_Z7CFFAC00(word(":") if (i_7 == 0) else (word("|")), e_1), to_list(delay(arrow_147)))))
            ...
            def arrow_171(_unit=None) -> Iterable[Doc]:
                ...
                def arrow_170(_unit=None) -> Iterable[Doc]:
                    def arrow_169(_unit=None) -> Iterable[Doc]:
                        def arrow_151(match_value_6: Tuple[str, Any]) -> Iterable[Doc]:
                            typename_0027 : str = type_renamer(match_value_6[0])
                            def arrow_150(_unit=None) -> Iterable[Doc]:
                                def arrow_149(match_value_7: Tuple[str, FSharpList[Tuple[str, monot]]]) -> Iterable[Doc]:
                                ...
                                return append_1(collect(arrow_149, to_array_1(match_value_6[1])), delay(lambda _unit=None: singleton(empty_2)))
                            
                            return append_1(singleton(word(to_text(interpolate("and %P() = ", [typename_0027])))), delay(arrow_150))
                        ...
                        def arrow_168(_unit=None) -> Iterable[Doc]:
                            def arrow_160(match_value_9: Tuple[str, Shape]) -> Iterable[Doc]:
                                ...
                                return append_1(singleton(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(word("and"), ret_t_1), word("=")), word("MK_" + typename_0027_1)), word("of unit"))), delay(arrow_152)) if (is_empty(fields_2)) else (append_1(singleton(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(word("and"), ret_t_1), word("=")), word("{"))), delay(arrow_159)))
                            
                            def arrow_167(_unit=None) -> Iterable[Doc]:
                                def arrow_165(match_value_12: Tuple[str, str, FSharpList[Tuple[str, str]], Doc]) -> Iterable[Doc]:
                                ...
                                return collect(arrow_165, doc_ctor_wrap_funcs)
                            
                            return append_1(collect(arrow_160, Sigma__GetRecordTypes(analyzer.Sigma)), delay(arrow_167))
                        
                        return append_1(collect(arrow_151, adt_cases), delay(arrow_168))
                    
                    return append_1(singleton(word("type ___used_t_head_90xasda")), delay(arrow_169))
                
                return append_1(singleton(empty_2), delay(arrow_170))
    ...
    with get_enumerator(lexer_maps) as enumerator:
        while enumerator.System_Collections_IEnumerator_MoveNext():
            ...
            if contains(k_3, analyzer.IgnoreSet):
                ...
                if contains_1(k_3, ReferencedNamedTokens, ObjectExpr178()):
