...
def resolve_macro(set_pos: Callable[[position], None], stmts: List[definition]) -> List[definition]:
    ...
    macro_defs : Any = empty(ObjectExpr27())
    ...
    def solve_specialization(tupled_arg: Tuple[dict[str, Any], Any], set_pos: Callable[[position], None]=set_pos, stmts: List[definition]=stmts) -> None:
        ...
        def arrow_35(tupled_arg: Tuple[dict[str, Any], Any]=tupled_arg) -> Iterable[Tuple[position, production]]:
            def arrow_34(match_value: Tuple[position, production]) -> Iterable[Tuple[position, production]]:
            ...
            return collect(arrow_34, decl_2["define"])
        
        define : FSharpList[Tuple[position, production]] = to_list(delay(arrow_35))
                ...
                def arrow_41(_unit=None) -> str:
                    ...
                    if match_value_2 is None:
                        ...
                        if match_value_3 is not None:
                            ...
                            scope_0027 : Any = of_list(map2(lambda k, v_1: (k, v_1), parameters, args_1), ObjectExpr40())
